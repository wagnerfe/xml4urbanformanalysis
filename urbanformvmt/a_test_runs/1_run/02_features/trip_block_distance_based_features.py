...
from ufo_map.Feature_engineering.block_v2 import features_blocks_distance_based
...
# path trip part
path_trip_part = '/data/metab/urbanformvmt/data/input/trip_parts'
path_buildings_part =  os.path.join(path_berlin,'Berlin_part_'+part+'_buildings_fts_buildings.csv')
path_trip_part = os.path.join(path_trip_part,'berlin_origin_trips_part_'+part+'.csv')
# path output
path_out = '/data/metab/urbanformvmt/data/output/' + 'berlin_trips_block_dist_ft_part_' + part + '.csv'
...
## Calculate features
# Calculate block distance based features
print('Computing block distance-based features...')
gdf_features_blocks_distance_based = features_blocks_distance_based(gdf_trip_origin_part_sample, gdf_buildings_part, buffer_sizes=[500,100])                                                                         
# Merge 
gdf_trip_origin_part_sample = gdf_trip_origin_part_sample.merge(gdf_features_blocks_distance_based,how='left',left_index=True,right_index=True)
